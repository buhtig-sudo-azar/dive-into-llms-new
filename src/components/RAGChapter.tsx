...
import { Database, Search, Sparkles, Wand2, FileText, ArrowRight, HelpCircle, Key, Cpu } from 'lucide-react';
...
const KNOWLEDGE_BASE: DocumentChunk[] = [
  ...
  },
  {
    ...
    text: 'Gemini от Google — это мультимодальные ИИ-модели следующего поколения, созданные работать со звуком, видео, кодом и картинками изначально. Окно контекста достигает 2 миллионов токенов. Родное ядро адаптировано под ультра-быструю TPU инфраструктуру.',
    tag: 'Gemini / Google'
  },
  {
    id: 'doc-3',
...
export default function RAGChapter() {
  ...
  const { getModelForRequest, getTokenForRequest } = useModelStore();
  ...
  const handleExecuteRAG = async () => {
    ...
    setTimeout(async () => {
      ...
      const computedDocs = KNOWLEDGE_BASE.map(doc => {
        ...
        keywords.forEach(kw => {
          if (docText.includes(kw)) score += 0.38;
        });
      ...
      try {
        ...
        console.error(err);
        setRagResult('Произошла ошибка при генерации.');
  ...
  const apiToken = getTokenForRequest();
  const hasKey = apiToken.length > 0;
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1 leading-relaxed">
              ...
              <strong className="text-emerald-400"> RAG (поиск и генерация)</strong> решает эту проблему: перед ответом система ищет
              соответствующие документы во внешней <strong className="text-emerald-400">векторной базе данных</strong>, встраивает
      ...
      {!hasKey && (
        <div className="bg-amber-950/20 border border-amber-500/30 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-semibold text-amber-300">Нужен API-ключ для демо</h4>
            <p className="text-xs text-amber-400/80 mt-1 leading-relaxed">
              Для работы RAG-демонстрации нужен токен OpenRouter. Нажмите кнопку <Cpu className="w-3 h-3 inline text-violet-400" /> выбора модели вверху страницы и добавьте свой ключ.
              Получить бесплатно: <a href="https://openrouter.ai/keys" target="_blank" rel="noopener noreferrer" className="text-amber-300 hover:underline">openrouter.ai/keys</a>
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-xl p-5 shadow-sm space-y-4">
          <span className="font-medium text-slate-200 flex items-center gap-2 border-b border-slate-800 pb-3 mb-1">
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start font-bold text-slate-200 mb-1">
                    <span>{doc.title}</span>
                    <span className="text-[9px] text-slate-500 uppercase bg-slate-800 px-1.5 py-0.5 rounded">
                      {doc.tag}
                    </span>
                  </div>
                  <p className="text-slate-400 leading-relaxed font-sans">{doc.text}</p>
                  ...
                  {doc.similarity !== undefined && (
                    <div className="mt-2 pt-2 border-t border-slate-800/50 space-y-1">
                      <div className="flex justify-between font-mono text-[9px] text-slate-500">
                        ...
                        <span className={sim >= 0.3 ? 'text-emerald-500 font-bold' : 'text-slate-500'}>
                          {(sim * 100).toFixed(0)}%
                        </span>
                      </div>
                      <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
                        <div
                          style={{ width: `${sim * 100}%` }}
                          className={`h-full ${sim >= 0.3 ? 'bg-emerald-500' : 'bg-slate-600'}`}
                        />
                      </div>
                    </div>
                  )}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5 shadow-sm space-y-4">
            <span className="font-medium text-slate-200 flex items-center gap-2 border-b border-slate-800 pb-3">
            ...
            <div className="flex gap-2 relative">
              <input
                ...
                className="flex-1 px-4 py-2 bg-slate-900 text-slate-200 border border-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-xs font-sans"
              />
              <button
            ...
            {/* Steps feedback */}
            {status !== 'idle' && (
              <div className="bg-slate-900 p-3 rounded-lg border border-slate-800/50 space-y-2 font-mono text-[10px]">
          ...
          {status === 'completed' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* With RAG */}
              <div className="bg-emerald-950/10 border border-emerald-500/40 rounded-xl p-4 space-y-2">
                ...
                <h4 className="font-semibold text-xs text-slate-200 mt-1">
                  Ответ на основе документов:
                </h4>
                <p className="text-xs text-slate-300 leading-relaxed font-sans bg-slate-900 border border-emerald-900/40 p-3 rounded-lg min-h-[96px]">
                  {ragResult}
                </p>
                <span className="text-[9px] text-emerald-500 font-sans italic block">
                  Проверено на соответствие оригинальным источникам.
                </span>
              </div>

              {/* Without RAG */}
              <div className="bg-rose-950/10 border border-rose-500/30 rounded-xl p-4 space-y-2">
                <span className="text-[10px] uppercase font-bold text-rose-400 flex items-center gap-1.5 font-mono">
                  <ArrowRight className="w-3.5 h-3.5" />
                  Без RAG (Риск галлюцинаций)
                </span>

                <h4 className="font-semibold text-xs text-slate-200 mt-1">
                  Свободный ответ модели:
                </h4>
                <p className="text-xs text-slate-300 leading-relaxed font-sans bg-slate-900 border border-rose-900/30 p-3 rounded-lg min-h-[96px]">
                  {noRagResult}
                </p>
                <span className="text-[9px] text-rose-400 font-sans italic block">
                  Может содержать вымышленные факты или устаревшие данные.
                </span>
              </div>
